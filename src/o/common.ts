import {BClient, qrcode} from "./BClient";
...
export const terminalClient = async (): Promise<BClient> => {
    await callSchoolList()
    const client: BClient = new BClient();
...
const events: Array<{ client: BClient, event: EventInfo }> = [];


export async function markEvent(client: BClient, event: string | number | EventInfo) {
